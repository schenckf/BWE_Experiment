...
#Lprev.model <- glmer(cbind(Lpresent, Labsent) ~ treatment * diversity, random = ~1 | bin / diversity, family = binomial, data = bwe.pot) #random effects model --> doesn't work :(
...
Lintensity.model <- lme(Lintensity ~ diversity * treatment, random = ~1 | bin / diversity, data = bwe.pot.Lintensity, method = "REML") #TARIK CODE using nlme package
...
#exploratory figures
p <- ggplot(bwe.pot, aes(x  = treatment, y = log(Cseverity+1), fill = diversity)) +
  scale_fill_manual(values = c("white", "grey")) +
  geom_violin(position = position_dodge(1)) +
  geom_dotplot(binaxis = 'y', stackdir = 'center', position = position_dodge(1), dotsize  = 0.5) +
  stat_summary(fun.y = mean, geom = "point", size = 3, color = "red", position = position_dodge(1)) +
...
##look at mono vs poly within temp as seperate analyses
...
hist(log(bwe.pot.Cintensity$Cintensity+1))
...
#Figure
p <- ggplot(bwe.pot.Cintensity, aes(x = treatment, y = log(Cintensity+1), fill = diversity)) + 
  scale_fill_manual(values = c("white", "grey")) +
  geom_violin(position = position_dodge(1)) +
  geom_dotplot(binaxis = 'y', stackdir = 'center', position = position_dodge(1), dotsize  = 0.5) +
  stat_summary(fun.y = mean, geom = "point", size = 3, color = "red", position = position_dodge(1)) +
