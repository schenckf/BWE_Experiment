...
```{r}
#Analyses
Lprev.model <-  glm(cbind(Lpresent, Labsent) ~ treatment + treatment:diversity, family = binomial, data = bwe.pot)

Lprev.model <- glm(cbind(Lpresent, Labsent) ~ treatment/diversity, family = binomial, data = bwe.pot)

Lprev.model <- glm(cbind(Lpresent, Labsent) ~ treatment + diversity %in% treatment, family = binomial, data = bwe.pot)

anova(Lprev.model, test = "LR")


Anova(Lprev.model) ##use this for P values (package = car)
###not working when trying to account for nested experimental design, glmer with (1|bin)?
...
Lsev.model2 <- aov(Lseverity ~ diversity * treatment, data = bwe.pot)
...
Lintensity.model <- aov(Lseverity ~ treatment * diversity, data = filter(bwe.pot, Lprevalence > 0))
summary(Lintensity.model)
plot(Lintensity.model)

#Figure
bargraph.CI(x.factor = treatment,
            response = Lseverity,
            data = filter(bwe.pot, Cprevalence > 0),
            ylim = c(0,20), ylab = "Mean lesion percent cover",
            xlab = "Temperature Treatment", legend = T)
...
Cprev.model <- glm(cbind(Cpresent, Cabsent) ~ treatment + diversity %in% treatment, family = binomial, data = bwe.pot)
