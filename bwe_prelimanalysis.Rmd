...
summary(Csev.model <- glm(Cseverity ~ diversity * treatment, family = poisson, data = bwe.pot))
...
bargraph.CI(x.factor = treatment,
            group = diversity,
            response = Cseverity+1,
            data = bwe.pot,
            ylim = c(0,1000), ylab = "disease cells/mg",
            xlab = "Temperature Treatment", legend = T)

##only for pots with disease -->intensity MODEL INCORRECT
Cintensity.model <- glm(Cseverity ~ treatment * diversity, family = poisson, data = filter(bwe.pot, Cprevalence > 0))
