...
```{r}
#Analyses
Lprev.model <- glm(cbind(Lpresent, Labsent) ~ diversity * treatment + treatment/rep, family = binomial, data = bwe.pot)
summary(Lprev.model)
...
#Csev.model.2 <- aov(Cseverity ~ diversity * treatment, data = bwe.pot)
...
Anova(Csev.model <- glm(Cseverity.round ~ diversity * treatment, family = poisson, data = bwe.pot))
#exploratory figures
bargraph.CI(x.factor = treatment,
